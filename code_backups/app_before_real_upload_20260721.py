...
def init_db():
    ...
    cur.execute(
        ...
        CREATE TABLE IF NOT EXISTS default_emission_factors (
            ...
            created_at TEXT NOT NULL,
            updated_at TEXT DEFAULT '',
            UNIQUE(country, energy_type, combustion_type)
        )
        """
    )
...
def load_default_factors():
    ...
    df = pd.read_sql_query(
        ...
        SELECT
            ...
            created_at,
            updated_at
        FROM default_emission_factors
        ORDER BY country, energy_type, combustion_type
        """,
        conn,
    )
    conn.close()
...
def get_default_factor(country, energy_type, combustion_type):
    conn = get_connection()
    cur = conn.cursor()
...
def update_default_factor(
    ...
    cur.execute(
        """
        INSERT INTO default_emission_factors
        (
            country,
            energy_type,
            combustion_type,
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT(country, energy_type, combustion_type)
        DO UPDATE SET
...
def copy_year_factors_to_default(source_year):
    now_text = datetime.now().isoformat(timespec="seconds")
...
def get_factor(factor_year, country, energy_type, combustion_type):
    ...
    # 2. 해당 연도 예외 배출계수가 없으면 기본 배출계수를 사용합니다.
    return get_default_factor(
        country=country,
...
def render_factor_page():
    st.subheader("온실가스 계수 관리")

    st.info(
         "국가, 에너지원, 연소형태로 구분되는 기본 배출계수와 순발열량 입니다. "
        "특정 연도에 계수가 변경되면 아래의 연도별 변경 계수 관리에 해당 연도를 등록한 뒤, "
        "선택 연도 재계산을 실행하세요."
    )

    factors_df = load_factors()

    st.subheader("기본 온실가스 계수 관리")

    st.caption(
        "국가 + 에너지원 + 연소구분 기준으로 관리합니다. "
        "연도별 변경 온실가스 계수가 없는 경우 이 기본값으로 계산됩니다."
    )

    with st.expander("기존 연도별 계수를 기본 계수로 복사", expanded=False):
        st.write(
            "선택한 연도의 연도별 계수를 기본 온실가스 계수로 복사합니다. "
            "기존 기본 계수가 있으면 선택한 연도 값으로 업데이트됩니다."
        )

        copy_source_year = st.number_input(
            "기본 계수로 복사할 연도",
            min_value=START_YEAR,
            max_value=MAX_YEAR,
            value=2026,
            step=1,
            key="copy_source_year_to_default"
        )

        confirm_copy_to_default = st.checkbox(
            f"{int(copy_source_year)}년 연도별 계수를 기본 온실가스 계수로 복사합니다.",
            key="confirm_copy_to_default"
        )

        if st.button(
            "기본 계수로 복사",
            type="primary",
            disabled=not confirm_copy_to_default,
            key="copy_year_to_default_button"
        ):
            copied_count = copy_year_factors_to_default(
                source_year=int(copy_source_year)
            )

            st.success(
                f"{int(copy_source_year)}년 계수를 기본 온실가스 계수로 복사했습니다. "
                f"처리 건수: {copied_count}건"
            )

            st.rerun()

    # with st.expander("연도별 변경 계수 전체 삭제", expanded=False):
    #     st.warning(
    #         "이 작업은 emission_factors 테이블에 저장된 모든 연도별 변경 계수를 삭제합니다. "
    #         "기본 온실가스 계수는 삭제되지 않습니다. "
    #         "삭제 후에는 연도별 변경 계수가 없는 모든 데이터가 기본 온실가스 계수로 계산됩니다."
    #     )

    #     confirm_delete_yearly_factors = st.checkbox(
    #         "연도별 변경 계수 전체 삭제에 동의합니다.",
    #         key="confirm_delete_yearly_factors"
    #     )

    #     if st.button(
    #         "연도별 변경 계수 전체 삭제",
    #         disabled=not confirm_delete_yearly_factors,
    #         key="delete_all_yearly_factors_button"
    #     ):
    #         deleted_count = delete_all_yearly_factors()

    #         st.success(
    #             f"연도별 변경 계수 {deleted_count}건이 삭제되었습니다. "
    #             "필요한 경우 특정 연도 변경 계수를 다시 등록하세요."
    #         )

    #         st.rerun()

 
    default_factors_df = load_default_factors()

    if default_factors_df.empty:
        st.info("등록된 기본 온실가스 계수가 없습니다.")
    else:
        display_default_df = default_factors_df[
            [
                "country",
                "energy_type",
                "combustion_type",
        ...
        display_default_df["net_calorific_value"] = display_default_df["net_calorific_value"].round(6)
        display_default_df["emission_factor"] = display_default_df["emission_factor"].round(6)
    ...
    with st.form("default_factor_form"):
        ...
        with default_col1:
            default_country = st.selectbox(
                "국가",
                ["한국", "멕시코", "베트남", "중국"],
                key="default_factor_country"
            )

        with default_col2:
            default_energy_type = st.selectbox(
                "에너지원",
                ENERGY_TYPES,
                key="default_factor_energy"
            )

        with default_col3:
            if default_energy_type == "전기":
                default_combustion_type = st.selectbox(
                    "연소구분",
                    ["고정연소"],
                    key="default_factor_combustion"
                )
            else:
                default_combustion_type = st.selectbox(
                    "연소구분",
        ...
        default_ncv = st.number_input(
            "순발열량",
            min_value=0.0,
            value=0.0,
            step=0.000001,
            format="%.10f",
            key="default_factor_ncv"
        )

        default_ef = st.number_input(
            "배출계수",
            min_value=0.0,
            value=0.0,
            step=0.000001,
            format="%.10f",
            key="default_factor_ef"
        )

        default_unit = st.text_input(
            "계수 단위 설명",
            value="",
            placeholder="예: 순발열량 MJ/m³, 배출계수 kgCO₂/TJ / 전기: 배출계수 tCO₂/MWh",
            key="default_factor_unit"
        )

        default_submitted = st.form_submit_button("기본 계수 저장")

        if default_submitted:
            update_default_factor(
                country=default_country,
                energy_type=default_energy_type,
                combustion_type=default_combustion_type,
                ncv=default_ncv,
                ef=default_ef,
                unit=default_unit,
            )
            st.success("기본 온실가스 계수가 저장되었습니다.")
            st.rerun()

    st.divider()

    st.subheader("연도별 변경 온실가스 계수 관리")


    if factors_df.empty:
        st.info("등록된 계수가 없습니다.")
    else:
    ...
    st.subheader("연도별 온실가스 계수 추가/수정")

    st.caption(
        "특정 연도에 기본 온실가스 계수와 다른 값을 적용해야 할 때만 입력합니다. "
        "해당 연도 변경 온실가스 계수가 없으면 기본 온실가스 계수가 사용됩니다."
    )

    with st.form("factor_form"):

        factor_row1_col1, factor_row1_col2 = st.columns(2)

        with factor_row1_col1:
        ...
        with factor_row1_col2:
            country = st.selectbox(
                "국가",
                ["한국", "멕시코", "베트남", "중국"]
            )

        factor_row2_col1, factor_row2_col2 = st.columns(2)

        with factor_row2_col1:
            energy_type = st.selectbox(
                "에너지원",
                ENERGY_TYPES
            )

        with factor_row2_col2:
            if energy_type == "전기":
                combustion_type = st.selectbox(
                    "연소구분",
                    ["고정연소"]
        ...
        submitted = st.form_submit_button("변경 계수 저장")
        ...
        if submitted:
            ...
            st.success("연도별 변경 온실가스 계수가 저장되었습니다.")
    ...
    st.write(
        "온실가스 계수를 수정한 뒤 이미 저장된 사용량 데이터의 배출량을 다시 계산합니다. "
        "재계산 전에는 DB 백업을 먼저 권장합니다."
    )

    st.warning(
        "재계산을 실행하면 activity_data 테이블의 emission_amount 값이 현재 온실가스 계수 기준으로 업데이트됩니다."
    )
    ...
    with recalc_col1:
        ...
        if st.button(
            ...
            st.success(
                ...
                f"온실가스 계수 누락: {result['missing_factor_count']}건"
            )
