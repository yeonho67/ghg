...
def render_factor_page():
    ...
    st.caption(
        ...
        "연도별 변경 계수가 없는 경우 이 기본값으로 계산됩니다."
    )
    ...
    st.subheader("연도별 변경 계수 추가/수정")

    st.caption(
        "특정 연도에 기본 온실가스 계수와 다른 값을 적용해야 할 때만 입력합니다. "
        "해당 연도 변경 계수가 없으면 기본 온실가스 계수가 사용됩니다."
    )
    ...
    st.warning(
        "재계산을 실행하면 activity_data 테이블의 emission_amount 값이 현재 계수 기준으로 업데이트됩니다."
    )
